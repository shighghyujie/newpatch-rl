...
def load_model(model_name, device):
    if(model_name == 'facenet'):
        ...
        insightface_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/insightface/insightface.pth'
        ...
        sphere20a_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/sphere20a/sphere20a.pth'
        ...
        sphere20a_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/re_sphere20a/re_sphere20a.pth'
        model = sphere20a(feature=True)
        #model.load_state_dict(torch.load(eval("{}_path".format(model_name)),map_location=device))
        model.eval()
        model = model.to(device)
        ...
        mobilefacenet_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/mobilefacenet/mobilefacenet_scripted.pt'
        #model = MobileFaceNet()
        #model.load_state_dict(torch.load(eval("{}_path".format(model_name)),map_location=device))
        ...
        arcface34_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/arcface34/arcface_34.pth'
        ...
        cosface34_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/cosface34/cosface_34.pth'
        model = iresnet34(False, dropout=0, fp16=True)
        model.load_state_dict(torch.load(eval("{}_path".format(model_name)),map_location=device))
        model.eval()
        model = model.to(device)
        ...
        arcface50_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/arcface50/ms1mv3_arcface_r50_fp16.pth'
        cosface50_path = 'E:/newpatch_rl/code_rl/rlpatch/stmodels/cosface50/glint360k_cosface_r50_fp16_0.1.pth'
...
def load_anchors(model_name, device, target):
    anchor_embeddings =  joblib.load('E:/newpatch_rl/code_rl/rlpatch/stmodels/{}/embeddings_{}_5752.pkl'.format(model_name,model_name))
...
def miattack_face(params_slove, model_names,
                  ...
                  width, height, emp_iterations, di, adv_img_folder, targeted = True):
    x, y = params_slove[0]
    weights = params_slove[2]
    epsilon = params_slove[3]
    nsig = params_slove[1]
    flag = 1 if targeted else -1
    w,h = img.size
    ...
    #print(X_ori.shape)
    delta = torch.zeros([1,3,8,10],requires_grad=True).to(device)
    delta2 = torch.zeros([1,3,25,30],requires_grad=True).to(device)
    # with torch.no_grad():
    #     delta[:,:,y:y+25,x:x+30] = initRandPatch()
    # with torch.no_grad():
    ...
    for itr in range(emp_iterations):   # iterations in the generation of adversarial examples
        # if itr%10 == 0 or itr == emp_iterations-1:
        #     patch1 = torch.tensor(delta[0,:,y:y+25,x:x+30])
        #     patch2 = patch1.cpu().numpy()
        #     patch2 = np.transpose(patch2,(2,1,0))
        #     patch3 = cv2.resize(patch2,(12,15))
        #     patch4 = cv2.resize(patch3,(25,30))
        #     patch4 = np.transpose(patch4,(2,1,0))
        #     patch5 = torch.from_numpy(patch4)
        #     with torch.no_grad():
        #         delta[0,:,y:y+25,x:x+30] = patch5
        delta2.retain_grad()
        X_adv = X_ori
        accm = 0
        patch1 = torch.tensor(delta[0])
        patch2 = patch1.cpu().numpy()
        patch2 = np.transpose(patch2,(2,1,0))
        ...
        with torch.no_grad():
            delta2[0,:,:,:] = patch4
        X_adv[0,:,y:y+25,x:x+30] = delta2[0,:,:,:]
        ...
        # 
        loss = flag * accm + 0.3*slope
        # loss = flag * accm
        # print('L_sim = {:.4f},L_slope = {:.4f}'.format(flag * accm.item(),0.3*slope.item()),end='\n')
        loss.backward(retain_graph=True)
        
        # MI operation
        grad_c = delta2.grad.clone()                    
        grad_c = F.conv2d(grad_c, TI_kernel(nsig), bias=None, stride=1, padding=(1,1), groups=3)   
        grad_a = grad_c / torch.mean(torch.abs(grad_c), (1, 2, 3), keepdim=True)+1.0*grad_momentum   # 1
        grad_momentum = grad_a # shape (1, 3, 160, 160)
        # grad_momentum = F.conv2d(grad_momentum, weight, stride=1, padding=1)
            
        delta2.grad.zero_()
        # 梯度乘以kernel
        delta2.data=delta2.data+epsilon * torch.sign(grad_momentum).to(device)
        #X_adv.data = X_adv.data 
        #delta.data=delta.data.clamp(-args.linf_epsilon/255.,args.linf_epsilon/255.)
        delta2.data=delta2.data.clamp(0,1)
        patch1 = torch.tensor(delta2[0])
        patch2 = patch1.cpu().numpy()
        patch2 = np.transpose(patch2,(2,1,0))
        patch3 = cv2.resize(patch2,(8,10))
        patch3 = np.transpose(patch3,(2,1,0))
        patch4 = torch.from_numpy(patch3)
        delta[0] = patch4
    patch1 = torch.tensor(delta[0])
    ...
    X_ori[0,:,y:y+25,x:x+30] = delta2[0,:,:,:]
...
# def check():
#     # faceimg = Image.open('E:/newpatch_rl/code_rl/rlpatch/example/guoying/1103_gy.jpg')
#     # crops_result, crops_tensor = crop_imgs([faceimg],args.width, args.height)
#     crops_result = [Image.open('E:/newpatch_rl/code_rl/rlpatch/adv_imgs/0.jpg')]
#     anchor_embeddings =  joblib.load('E:/newpatch_rl/code_rl/rlpatch/stmodels/{}/embeddings_{}_5752.pkl'.format(args.source_model,args.source_model))
