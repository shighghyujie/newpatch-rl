...
def vector_processor(location, featuremap, eps_logits,space,device):
    """
    Args: 
    ...
    op = location.reshape(1, 1, -1)  # (1, 1, h*w)
    #print('op = ',op,op.shape)
    loct_resp = torch.softmax(op,dim=2)           # (1,1,h*w)
    ...
    '''-------------------kernel action-------------------'''
    # fm_op = featuremap2[0] 
    # op = fm_op[:9].reshape(9,1,-1)  # (n_models,1,h*w)
    # weg_resp = torch.mean(op,dim=2).t()            # (1,n_models)
    # weg_probs = torch.softmax(weg_resp,dim=1)      # (1,n_models)
    # for i in range(9):
    #     dist_weg = Normal(weg_probs[0][i], torch.tensor(0.02).to(device))
    ...
    #print('eps_probs:',eps_probs)
    '''-------------------kernel action-------------------'''
    # eps_probs2 = torch.softmax(eps_logits2,dim=1)      # (bt,eps_dim)
    # #print('eps_probs = ',eps_probs)
    # dist_eps2 = Categorical(eps_probs2[0])
    # pre_actions.append(dist_eps2)
